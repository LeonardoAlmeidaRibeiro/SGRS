<script>
    ...
    function executarModalEditar() {
        ...
        if (nome == '') {
            Swal.fire({
                icon: 'error',
                title: 'Oops...',
                text: 'Preencha o campo Forma de Conhecimento do Evento'
            });
            return false;
        }

        $.ajax({
            url: "{{ route('conhecimentoEvento.update', '') }}/" + id,
            type: "PUT",
            data: "&nome=" + nome,
            headers: headers,
            error: function(data) {
    ...
    function executarModalCriar() {
        ...
        $.ajax({
            url: "{{ route('conhecimentoEvento.store') }}",
            type: "POST",
            data: "&nome=" + nome,
            headers: headers,
            error: function(data) {
    ...
    function excluir(id) {
        ...
        Swal.fire({
            ...
            if (typeof(result.value) != "undefined" && result.value == true) { // Se foi apertado o botão de "Sim, excluir"
                ...
                $.ajax({
                    url: "{{ route('conhecimentoEvento.destroy', '') }}/" + id,
                    type: "DELETE",
                    headers: headers,
                    success: function(data) {
